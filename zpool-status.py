...
def check_zpool_status():
    try:
        result = subprocess.run(["zpool", "status"], capture_output=True, text=True, check=True)
...
def format_message(warnings, full_output):
    if warnings:
        header = "**🚨 OBSIDIAN ZFS Zpool Alert**\nThe following issues were found:\n"
        warning_text = "\n".join(f"- `{w}`" for w in warnings)
    else:
        header = "**✅ OBSIDIAN ZFS Zpool Status: All pools are healthy.**\n"
        warning_text = ""
...
def main():
    ...
    # Send both Discord and Email notifications simultaneously
    try:
        send_discord_alert(message)
    except Exception as discord_error:
        print(f"Discord alert failed: {discord_error}")
    
    send_email_backup("ZFS Zpool Monitor Update", message)  # Send email regardless of error
